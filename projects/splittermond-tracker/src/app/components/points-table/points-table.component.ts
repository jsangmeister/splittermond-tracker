...
export class PointsTableComponent {
    ...
    public plus(value: string) {
        this.change(value, -1);
    }

    public minus_channeled() {
        if (this.mode() === "splinters") {
            this.change("k1");
        }
    }

    public plus_channeled() {
        if (this.mode() === "splinters") {
            this.change("k1", -1);
        }
    }
    ...
    private change(value: string, factor = 1) {
        try {
            ...
            if (this.mode() === 'lp') {
                ...
                if (res) {
                    ...
                    if (other > 0 && other < consumed) {
                        throw new Error("Invalid format: consumed cannot be larger than the first value");
                    }
                    return {
